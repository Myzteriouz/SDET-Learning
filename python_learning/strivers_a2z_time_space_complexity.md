...
- In competitive programming or in the platforms like takeUforward, we generally run our codes on online servers. Most of these servers execute roughly 108 operations in approximately 1 second i.e. 1s.
- We must be careful that if the time limit is given as 2s the operations in our code must be roughly 2*108, not 1016.
- Similarly, 5s refers to 5*108.
